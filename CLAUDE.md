...
- **`rtl_rdp/`** — The RDP product (published as a dbt package, consumed by clients)
- **`rtl_rdp_client/`** — Customer implementation layer (maps source data to the RDP contract)
...
All dbt commands run from within the relevant project directory (`rtl_rdp/` or `rtl_rdp_client/`).

```bash
dbt deps          # Install packages (required before first run in rtl_rdp_client)
...
> Note: `rtl_rdp/.claude/settings.local.json` only permits `dbt compile` by default.
...
Customers implement staging models in `rtl_rdp_client/models/staging/`. The contract is defined in `rtl_rdp/CONTRACT.md`. Key rules:
...
Full conventions are in `rtl_rdp/STYLE_GUIDE.md`. Key rules:
